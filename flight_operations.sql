...
SELECT airline,price,
  ROUND(AVG(price) OVER (PARTITION BY airline ORDER BY indexs ROWS BETWEEN 2 PRECEDING AND CURRENT ROW),2) AS moving_avg_price
FROM airlines;
...
SELECT airline,source_city,destination_city,days_left,price,
       price - LAG(price) OVER (PARTITION BY source_city, destination_city ORDER BY days_left DESC) AS price_diff_prev
FROM airlines;
...
-- Q10. Divide all flights into 4 price quartiles
-- Function [NTILE(n)-Divides rows into n equal buckets]
...
SELECT airline,source_city,destination_city,days_left,price,
       FIRST_VALUE(price) OVER (PARTITION BY source_city, destination_city ORDER BY days_left DESC) AS first_price,
       LAST_VALUE(price) OVER (PARTITION BY source_city, destination_city ORDER BY days_left DESC ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS last_price
FROM airlines;

-- Q12.Show 3rd cheapest flight per airline
--[NTH_VALUE() Returns the nth value in the window]
SELECT airline,source_city,destination_city,days_left,price,
       NTH_VALUE(price, 3) OVER (PARTITION BY airline ORDER BY price ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS third_cheapest
FROM airlines;
